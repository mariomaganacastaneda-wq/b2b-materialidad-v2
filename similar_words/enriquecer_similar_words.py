"""
===============================================================================
  SEICO B2B - Enriquecimiento MASIVO de similar_words (v3 OPTIMIZADO)
  Claude Haiku 4.5 + Prompt v4
  53,057 registros del catalogo SAT
===============================================================================
  ...
  Uso:
    ...
    python enriquecer_similar_words.py --test 50      # Solo 50 registros
    python enriquecer_similar_words.py --resume       # Reanuda
    python enriquecer_similar_words.py --dry-run      # No actualiza BD
===============================================================================
"""
...
BATCH_SIZE = 30           # Subido de 15 a 30 (Haiku lo maneja bien)
MAX_RETRIES = 5
RATE_LIMIT_PAUSE = 0.3    # Bajado de 1.0 a 0.3s
OUTPUT_DIR = Path(".")
...
# === Supabase REST (optimizado con batch update) ==============================
...
class SupabaseREST:
    ...
    def update_one(self, table, code, data):
        """UPDATE una fila."""
        r = self.client.patch(
            "{}/rest/v1/{}".format(self.url, table),
            params={"code": "eq.{}".format(code)},
    ...
    def update_batch_parallel(self, table, results, max_workers=10):
        """UPDATE multiples filas en paralelo con ThreadPoolExecutor."""
        updated = 0
        errors = 0

        def do_update(code, sw):
            try:
                self.update_one(table, code, {"similar_words": sw})
                return True
            except Exception:
                return False
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for rec in results:
                ...
                if code and sw:
                    f = executor.submit(do_update, code, sw)
                    futures[f] = code
...
def call_haiku(client, batch, attempt=1):
    try:
        response = client.messages.create(
            model=MODEL,
            max_tokens=5000,       # Mas tokens para batch de 30
            temperature=0.4,
            system=PROMPT_V4,
        ...
        print("\n    [Rate limit - {}s]".format(wait), end=" ", flush=True)
        time.sleep(wait)
...
def main():
    parser = argparse.ArgumentParser(description="Enriquecimiento masivo similar_words (v3)")
    parser.add_argument("--test", type=int, help="Solo N registros de prueba")
    parser.add_argument("--resume", action="store_true", help="Reanudar desde progreso")
    parser.add_argument("--dry-run", action="store_true", help="No actualizar BD")
    ...
    print()
    print("=" * 70)
    print("  SEICO B2B - Enriquecimiento Masivo similar_words (v3 OPTIMIZADO)")
    print("  Modelo: {} | Batch: {}".format(MODEL, batch_size))
    print("  {}".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    if args.test:
        print("  MODO TEST: {} registros".format(args.test))
    if args.dry_run:
        print("  MODO DRY-RUN")
    if args.resume:
        print("  MODO RESUME")
    ...
    if not ANTHROPIC_API_KEY:
        print('\nERROR: $env:ANTHROPIC_API_KEY="sk-ant-..."')
        sys.exit(1)
    if not SUPABASE_SERVICE_KEY:
        print('\nERROR: $env:SUPABASE_SERVICE_KEY="eyJ..."')
        sys.exit(1)
    ...
    if args.test:
        ...
        print("Muestra: {} registros".format(len(all_records)))

    # Progreso
    progress = load_progress() if args.resume else {
        "processed_codes": {}, 
        "stats": {"input_tokens": 0, "output_tokens": 0, "errors": 0}
    }
    ...
    if args.resume and progress["processed_codes"]:
        ...
        print("Resume: {:,} ya procesados, {:,} pendientes".format(skipped, len(all_records)))
    ...
    print("\nProcesando {:,} registros en {:,} batches de {}...".format(
        total_records, total_batches, batch_size))
    print("  (Ctrl+C para pausar y guardar progreso)\n")
    ...
    try:
        for i, batch in enumerate(batches):
            ...
            eta_min = ((total_records - processed) / rate / 60) if rate > 0 else 0

            # Linea de progreso compacta
            eta_str = " | ETA:{:.0f}m".format(eta_min) if i > 2 else ""
            cost_so_far = (progress["stats"]["input_tokens"] / 1e6) * 1.00 + (progress["stats"]["output_tokens"] / 1e6) * 5.00
            cost_str = " | ${:.2f}".format(cost_so_far) if i > 0 else ""

            print("  [{:>5.1f}%] {}/{}{}{}...".format(
                pct, i + 1, total_batches, eta_str, cost_str), end=" ", flush=True)
            ...
            # Update Supabase en paralelo
            if args.dry_run:
                n_upd = len(results)
                n_err = 0
            else:
                n_upd, n_err = db.update_batch_parallel(
                    "cat_cfdi_productos_servicios", results, max_workers=10)
            ...
            processed += len(batch)
            tok = resp.get("input_tokens", 0) + resp.get("output_tokens", 0)

            print("{}/{} | {:,}t".format(len(results), len(batch), tok))

            # Guardar progreso cada 20 batches
            if (i + 1) % 20 == 0:
        ...
        save_progress(progress)
        print("  Guardado. Usa --resume para continuar.\n")
    ...
    # Estadisticas
    elapsed = time.time() - start_time
    total_in = progress["stats"]["input_tokens"]
    total_out = progress["stats"]["output_tokens"]
    ...
    print("  RESULTADO")
    print("=" * 70)
    print("  Procesados:  {:,} / {:,}".format(processed, total_records))
    print("  Actualizados:{:,}".format(updated_db))
    print("  Errores:     {:,}".format(errors))
    print("  Tokens:      {:,} (in:{:,} out:{:,})".format(total_tok, total_in, total_out))
    print("  Costo:       ${:.2f} USD".format(cost))
    print("  Tiempo:      {:.1f} min".format(elapsed / 60))
    if elapsed > 0 and processed > 0:
        print("  Velocidad:   {:.1f} reg/s".format(processed / elapsed))
        full_eta = (53057 / (processed / elapsed)) / 60
        print("  ETA total:   ~{:.0f} min para 53K registros".format(full_eta))
    if args.dry_run:
        print("  ** DRY-RUN **")
    print("=" * 70)
